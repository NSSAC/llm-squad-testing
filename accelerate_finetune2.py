...
OUTPUT_DIR = "/scratch/gjf3sa/llama8b_lora_accerate_entireModel2"

lora_cfg = LoraConfig(
    r             = 16,           # tiny adapters → small VRAM
    lora_alpha    = 32,          # ≈ 2 × r
    lora_dropout  = 0.1,
...
def to_prompt(ex):
    ...
    prompt = (
        ...
        "You are a precise question-answering assistant. Your task is to extract the exact span from the context that answers the question. "
        "Do not paraphrase or generate information not explicitly in the context. "
        "If no answer exists in the context, respond with only 'Unanswerable'.\n"
        "<|user|>\n"
        f"Context: {ctx}\n\n"
        f"Question: {q}\n"
        "<|assistant|>\n"
        f"Answer: {ans_txt}<|end_of_text|>"
    )
    return {"text": prompt}
